    ...
    respond_to do |format|
      if params[:withdraw].present?
        if params[:withdraw].to_f > current_user.balance.to_f
          ...
          current_user.user_histories.create(status: params[:commit], amount: params[:withdraw].to_f)
        ...
        current_user.user_histories.create(status: params[:commit], amount: params[:deposit].to_f)
        ...
        format.html { redirect_to root_path, notice: 'Balance remain unchanged' }
      end
    end
  end
end
